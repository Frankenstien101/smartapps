...
<div class="admin-reports-container">
    ...
    <!-- Add this after the filter card to show current branch -->
<div class="alert alert-info" id="currentBranchDisplay" style="display: none; margin-bottom: 20px;">
    <i class="fas fa-store"></i> Currently viewing: <strong id="branchNameDisplay">All Branches</strong>
</div>
...
<script>
// API Configuration
const API_URL = '/SIDJAN/datafetcher/admindata.php';
...
async function loadSalesReport() {
    ...
    if (result.success) {
        // Update chart titles to show branch
        const salesChartTitle = document.querySelector('#salesTab .chart-container:first-child h5');
        if (salesChartTitle) {
            salesChartTitle.innerHTML = `<i class="fas fa-chart-line"></i> Daily Sales Trend ${branch !== 'all' ? `- ${branchName}` : '- All Branches'}`;
        }
        
        const topProductsTitle = document.querySelector('#salesTab .chart-container:last-child h5');
        if (topProductsTitle) {
            topProductsTitle.innerHTML = `<i class="fas fa-crown"></i> Top Selling Products ${branch !== 'all' ? `- ${branchName}` : '- All Branches'}`;
        }
        ...
        // Update charts
        updateSalesChart(result.daily_breakdown || [], branchName);
        updateTopProductsChart(result.top_products || [], branchName);
    }
}

// Update chart functions to show branch info
function updateSalesChart(data, branchName) {
...
function updateTopProductsChart(data, branchName) {
    const ctx = document.getElementById('topProductsChart').getContext('2d');
    if (topProductsChart) topProductsChart.destroy();
    
    // Limit to top 5 for better display
    const top5 = data.slice(0, 5);
    
    topProductsChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: top5.map(d => d.ProductName.length > 20 ? d.ProductName.substring(0,20)+'...' : d.ProductName),
            datasets: [{
                label: `Revenue (₱) - ${branchName}`,
                data: top5.map(d => d.TotalAmount),
...
function updateBranchDisplay() {
    ...
    const selectedBranch = branchSelect.options[branchSelect.selectedIndex]?.text || 'All Branches';
    const displayDiv = document.getElementById('currentBranchDisplay');
    const branchNameSpan = document.getElementById('branchNameDisplay');
    
    if (displayDiv && branchNameSpan) {
        branchNameSpan.innerText = selectedBranch;
...
async function applyFilters() {
    updateBranchDisplay(); // Add this line
    await loadDashboardSummary();
...
// Also update loadBranches function
async function loadBranches() {
    const result = await apiCall('getBranches');
    if (result.success && result.data) {
        const select = document.getElementById('branchSelect');

<?php
// pages/mobile-phones.php - Products Inventory Page with Serialized Support
?>
<style>
...
<div class="mobile-phones-container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4><i class="fas fa-boxes"></i> Products Inventory</h4>
            <p class="text-muted mb-0">Manage your entire product inventory with serialized tracking</p>
        </div>
        <div>
            <button class="btn btn-primary"  style= "display: none;" data-bs-toggle="modal" data-bs-target="#addModal">
                <i class="fas fa-plus" style= "display: none;"></i> Add Product
            </button>
        </div>
    </div>
    ...
    <div class="filter-section">
        <div class="search-box">
            ...
            <input type="text" id="searchInput" placeholder="Search by name, category, code, IMEI, or Serial...">
        </div>
        <div class="filter-buttons" id="categoryFilterContainer">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-plus-circle"></i> Add Product</h5>
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="addProductCategory" class="form-control" placeholder="e.g., Mobile Phones">
                </div>
                <div class="mb-3">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" id="addDiscount" class="form-control" step="0.01" placeholder="0" min="0" max="100">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Less (₱)</label>
                        <input type="number" id="addLess" class="form-control" step="0.01" placeholder="0.00" min="0">
                    </div>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea id="addProductDesc" class="form-control" rows="2" placeholder="Product description, specs, etc."></textarea>
                </div>
            </div>
...
<!-- DELETE CONFIRMATION MODAL -->
<div class="modal fade modal-danger" id="deleteModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
...
async function deleteProduct(productId) {
    const result = await apiCall(API_URL, 'deleteProduct', 'DELETE', { product_id: productId });
    if (result.success) {
...
async function loadBrands() {
    // We don't need brands anymore, but keep for compatibility
    allBrands = [];
}
...
function renderCategoryFilters() {
    ...
    let html = '<button class="filter-btn active" data-category="all">All Categories</button>';
    
    allCategories.forEach(category => {
        html += `<button class="filter-btn" data-category="${escapeHtml(category)}">${escapeHtml(category)}</button>`;
    });
    
    container.innerHTML = html;
    
    // Re-attach event listeners
    document.querySelectorAll('#categoryFilterContainer .filter-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            document.querySelectorAll('#categoryFilterContainer .filter-btn').forEach(b => b.classList.remove('active'));
...
async function addProduct() {
    ...
    const less = parseFloat(document.getElementById('addLess').value) || 0;
    const productCode = document.getElementById('addProductCode').value || 'PROD-' + Date.now();
...
function confirmDelete(productId, productName) {
    ...
    document.getElementById('deleteProductName').innerHTML = `<strong>${escapeHtml(productName)}</strong>`;
...
function getProductImageHtml(product) {
    if (product.ProductImagePath && product.ProductImagePath !== '') {
        return `<img src="${product.ProductImagePath}" 
                       ...
                       onerror="this.parentElement.classList.add('default-bg'); this.style.display='none'; this.parentElement.innerHTML='<i class=\'fas fa-box\'></i>';">`;
    } else {
...
function displayProducts(products) {
    ...
    container.innerHTML = products.map(product => {
        ...
        let discountBadges = '';
        if (discount > 0) {
            discountBadges += `<span class="badge-discount" style="background:#dc3545;color:white;padding:2px 6px;border-radius:4px;font-size:10px;margin-right:3px;">-${discount}%</span>`;
        }
        if (less > 0) {
            discountBadges += `<span class="badge-less" style="background:#d97706;color:white;padding:2px 6px;border-radius:4px;font-size:10px;">-₱${formatNumber(less)}</span>`;
        }
            ...
            <div class="product-card">
                ...
                <div class="product-info">
                    <div class="product-name">
                        ...
                        ${hasDiscount ? ` <span style="font-size:10px;">${discountBadges}</span>` : ''}
                    </div>
                    <div class="product-code">Code: ${product.ProductCode || 'N/A'}</div>
                    <div class="product-category"><i class="fas fa-tag"></i> ${product.Category || 'Uncategorized'}</div>
                    <div class="product-price">
                        ${hasDiscount ? `<span style="text-decoration:line-through;color:#999;font-size:14px;font-weight:400;">₱${formatNumber(sellingPrice)}</span> ` : ''}
                    ...
                    <div class="product-actions">
                        ${hasUnits ? `
                            <button class="btn-sm btn-outline-info" onclick="viewUnits(${product.ProductID}, '${escapeHtml(product.ProductName)}')">
                                <i class="fas fa-list"></i> Units
                            </button>
                        ` : ''}
                        <button class="btn-sm btn-outline-secondary" onclick="openEditModal(${product.ProductID})">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button class="btn-sm btn-outline-danger" onclick="confirmDelete(${product.ProductID}, '${escapeHtml(product.ProductName)}')">
...
function openEditModal(productId) {
    ...
    // Build dynamic edit modal with no brand
    const modalHtml = `
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Category *</label>
                            <input type="text" id="editProductCategory" class="form-control" value="${escapeHtml(product.Category || '')}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Product Code</label>
                            <input type="text" id="editProductCode" class="form-control" value="${escapeHtml(product.ProductCode || '')}">
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Cost Price (₱)</label>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Discount (%)</label>
                                <input type="number" id="editDiscount" class="form-control" step="0.01" value="${product.Discount || 0}" min="0" max="100">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Less (₱)</label>
                                <input type="number" id="editLess" class="form-control" step="0.01" value="${product.Less || 0}" min="0">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Description</label>
                            <textarea id="editProductDesc" class="form-control" rows="2">${escapeHtml(product.Description || '')}</textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
    ...
    document.getElementById('confirmEditBtn').addEventListener('click', async function() {
        const data = {
            ...
            less: parseFloat(document.getElementById('editLess').value) || 0,
...
function clearAddForm() {
    ...
    document.getElementById('addLess').value = '';

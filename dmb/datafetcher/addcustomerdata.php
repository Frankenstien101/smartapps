...
function getCustomers($conn, $currentBranch) {
    ...
    $query = "
        ...
        SELECT TOP $limit
            ...
            Branch,
            ISNULL((SELECT COUNT(*) FROM Sales WHERE CustomerPhone = c.Phone AND Branch = @branch), 0) + ISNULL((SELECT COUNT(*) FROM Installments WHERE CustomerName = c.CustomerName AND Branch = c.Branch AND Branch = @branch), 0) AS TotalPurchases,
            ISNULL((SELECT SUM(TotalAmount) FROM Sales WHERE CustomerPhone = c.Phone AND Branch = @branch), 0) + ISNULL((SELECT SUM(PaidAmount) FROM Installments WHERE CustomerID = c.customerID AND Status = 'Paid' AND Branch = @branch), 0) + ISNULL((SELECT SUM(DownPayment) FROM Installments WHERE CustomerID = c.customerID AND Branch = @branch), 0) AS TotalSpent
        FROM Customers c

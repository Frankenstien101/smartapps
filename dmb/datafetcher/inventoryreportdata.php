...
function getInventorySummary($conn, $currentBranch) {
    $query = "SELECT 
                ...
                COUNT(CASE WHEN AvailableQuantity < 10 AND AvailableQuantity > 0 THEN 1 END) AS LowStockCount,
                COUNT(CASE WHEN AvailableQuantity >= 10 THEN 1 END) AS WellStockedCount
              FROM Products
              WHERE Branch = :branch";
...
function getLowStockReport($conn, $currentBranch) {
    ...
    $summary = [
        ...
        'total_units' => array_sum(array_column($products, 'AvailableQuantity'))
    ];
